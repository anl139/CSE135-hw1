...
// Determine the EFFECTIVE HTTP method (supports POST + _method override)
function effective_method() {
    ...
    if ($method === 'POST') {
        if (!empty($_POST['_method'])) {
            ...
            if (in_array($override, ['PUT', 'DELETE', 'PATCH'])) return $override;
        }
...
// --------------------------------------------------
// Parse incoming data for any method
// --------------------------------------------------
$data = [];
...
// Remove _method from echoed data
unset($data['_method']);
